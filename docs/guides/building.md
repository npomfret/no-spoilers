...
- Xcode Cloud archives **both** schemes on every push to `main` and uploads both: `NoSpoilersApp` for iOS and `NoSpoilers` for macOS, two ARCHIVE actions in one workflow. **The build arrives attached to no tester group and nobody can install it until it is put in one** — that is a command somebody runs, deliberately, not a post-action. Task 14 Phase 1 step 6 is the argument. Its hook lives in `NoSpoilers/ci_scripts/`, beside the Xcode project — Xcode Cloud ignores a `ci_scripts` directory at the repository root.
  ...
  - **`scripts/testflight_distribute.py` is still iOS-only** (`asc.TESTFLIGHT_PLATFORM = "IOS"`, whose comment says the Mac app "has no TestFlight story at all" — that is now out of date). A macOS CI build will upload, process, and sit in no tester group, which is indistinguishable from success from every other signal. Generalising the platform is the second half of option D and is not done.
