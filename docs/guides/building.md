...
- **Three tag families, since task 32 (2026-09-05), and each means one thing.** Before it there was one, `vX.Y.Z`, written at the *first* upload of a train, which is not the build users got: `v1.1.2` marks `bump to v1.1.2 (build 10003)` and the build on sale is 10012. Those bare tags are left where they are — the Homebrew cask and the GitHub releases point at them — and mean "first upload of that train". Two of them did not: `v1.1.2` and `v1.1.3` were written by that old rule at an *App Store* upload, and no Developer ID release was ever made of either, so they were deleted on 2026-09-09 rather than left to be silently reused by `tag_version`, which skips a name that already exists. The bare family now ends at `v1.1.1` (2026-08-12), the last Developer ID release. Going forward:
